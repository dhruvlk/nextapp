...
const returnBook = async (_, args, ctx) => {
    try {
        ...
        if (moment(borrowInstance.expiryDate).isBefore(moment(data.returnDate).add(-1, 'days'))) {
            fineAmount = CONFIG.fine;
        }
        if (moment(borrowInstance.expiryDate).isBefore(moment(data.returnDate).subtract(5, 'days'))) {
            console.log("70")
